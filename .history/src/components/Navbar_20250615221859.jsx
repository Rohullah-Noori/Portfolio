...
function Navbar() {
  ...
  const currencthash = location.hash;
        ...
        <div className="flex items-center justify-end p-4 ">
          <nav className="flex items-center justify-between w-full ">
            ...
            <ul
              ...
              } flex flex-col md:flex-row items-start px-12 pt-2 space-y-3 md:space-y-0 md:space-x-4 w-full md:w-auto`}
            >
              {Links.map((link) => (
                <li
                  key={link.name}
                  className={` relative group font-display  border-b-1 border-transparent text-md sm:text-md md:text-md lg:text-lg   hover:text-blue-400   ${
                    currencthash === link.path ? "text-blue-400 " : ""
                  }`}
                >
                  <a href={link.path}>
                    {link.name}
                  ...
                  {/* <ScrollLink
                    to={link.path.replace("#", "")} // تبدیل "#about" به "about"
                    key={link.name}
                    spy={true}
                    smooth={true}
                    offset={-70}
                    duration={500}
                    className={`cursor-pointer relative group font-display text-md hover:text-blue-400 ${
                      currencthash === link.path ? "text-blue-400 " : ""
                    }`}
                  >
                    {link.name}

      ...
      <Container>
        <div className="flex justify-end items-center p-4 ">
          <nav className="flex justify-between items-center w-full ">
            ...
            <ul
              className={`   md:flex md:space-x-4 text-black absolute md:static bg-white md:bg-transparent left-0 md:w-auto md:opacity-100 transition-all duration-300 ease-in-out  ${
                open ? "top-[60px] opacity-[100]" : "top-full opacity-0"
              } flex flex-col md:flex-row items-start px-8 pt-2 space-y-3 md:space-y-0 md:space-x-4 w-full md:w-auto`}
            >
              {Links.map((link) => (
                <li
                  key={link.name}
                  className={` relative group font-display  border-b-1 border-transparent text-lg sm:text-xl md:text-2xl lg:text-3xl   hover:text-blue-400   ${
